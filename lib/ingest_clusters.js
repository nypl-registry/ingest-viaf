	...
	return function(callback){
		...
		if (cluster.isMaster) {
			...
			process.on('message', msg => {
				...
				setTimeout(function(){ db.databaseClose();  process.exit();  },   Math.floor(Math.random() * (10000 - 1000 + 1)) + 1000)
				...
				_(fs.createReadStream(msg.work))
					...
					.map(xmlLine => {
						...
						//WE ARE GOING FULLLL STRINGS DAWG.
			...
			//ask for the file name, but stagger the startup
			setTimeout(function(){
