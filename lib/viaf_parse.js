...
exports.returnPersist = function (xml) {
  ...
  if (!oldId || !newId) {
    ...
    return { oldId: oldId, newId: newId }
  }
}
...
exports.returnAgent = function (viafId, xml) {
  ...
  if (agent.lcTerm) {
    n = utils.normalizeAndDiacritics(agent.lcTerm)
    if (n.length > 2) if (agent.normalized.indexOf(n) === -1) agent.normalized.push(n)
  }
  if (agent.dnbTerm) {
    n = utils.normalizeAndDiacritics(agent.dnbTerm)
    if (n.length > 2) if (agent.normalized.indexOf(n) === -1) agent.normalized.push(n)
  }
  if (agent.viafTerm) {
    n = utils.normalizeAndDiacritics(agent.viafTerm)
    if (n.length > 2) if (agent.normalized.indexOf(n) === -1) agent.normalized.push(n)
  }
